...
proficiency. Unfortunately, that feeling has shifted towards another language as of late.
...
Systems Programming covered the C programming language in some detail and how to design concurrent/parallel[^wut] code.
...
After reaching that conclusion, I also realised that Java's whole design philosophy doesn't match my personal interests
anymore. The _whole point_ of a language like Java is that you don't have to worry about pure optimisation or managing
memory. Whereas in a language like Rust, the challenges are closer to the hardware. The utterly different memory model
involves reasoning about your code entirely differently.
...
more [pattern matching](https://openjdk.java.net/jeps/420) [^amber] features as well which shows that Java is getting
inspiration from other languages and implementing its own twist on it.

In my humble opinion, I think the Java programming language is in a really great position to continue dominating the
market as it has been for the past few decades... and innovating along the way!

## So Wait... Do You Like Java Or Not?

The title may be _slightly_ misleading, if I was to rewrite the title right now. I would make it...
